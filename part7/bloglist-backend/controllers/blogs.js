...
blogsRouter.post('/:id/comments', userExtractor, async (request, response) => {
	if (!request.body.content) {
		response.status(400).json({ error: 'empty comment' })
		return
	}
	const result = await Blog.findByIdAndUpdate(request.params.id, {$push: { comments: request.body.content }}, { safe: true, upsert: true, new: true })
